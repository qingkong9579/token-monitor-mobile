...
- **首页 GLM/Z.ai 卡片的滚动额度不再重复显示**：v0.56 起 GLM 行会把 ZCode 登录的额度并入（上游 #630），同一卡片可能携带多条同类型窗口（两条 5-hour、两条 Weekly）；首页此前原样列出导致重复。现在首页按桌面额度视图的选择语义取窗口——session/weekly 各一条、Start Daily 等 daily 窗口与 Start 计划桶全保留、MCP 与现金余额各一条
